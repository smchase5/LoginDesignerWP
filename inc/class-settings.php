...
class LoginDesignerWP_Settings
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_logindesignerwp_upload_image', array($this, 'ajax_upload_image'));
    }
    ...
    public function enqueue_admin_assets($hook)
    {
        ...
        // This handles all media dependencies (backbone, underscore, wp-hooks, media-views, etc.)
        wp_enqueue_media();

        // React Admin App (Vite Build)
        $build_url = LOGINDESIGNERWP_URL . 'assets/build/';
        $build_path = LOGINDESIGNERWP_PATH . 'assets/build/';

        $js_ver = file_exists($build_path . 'admin.js') ? filemtime($build_path . 'admin.js') : LOGINDESIGNERWP_VERSION;
        $css_ver = file_exists($build_path . 'admin.css') ? filemtime($build_path . 'admin.css') : LOGINDESIGNERWP_VERSION;
        ...
        wp_enqueue_script(
            'logindesignerwp-admin',
            $build_url . 'admin.js',
            array('jquery', 'wp-color-picker', 'media-views'), // Simplified deps
            $js_ver,
            true
        );
        ...
        if (!empty($settings['logo_id'])) {
            $settings['logo_image_url'] = wp_get_attachment_image_url($settings['logo_id'], 'medium');
        }
        ...
        if (class_exists('LoginDesignerWP_Security')) {
        ...
        wp_localize_script('logindesignerwp-admin', 'logindesignerwpData', array(
            ...
            'i18n' => array(
                'saved' => __('Settings saved successfully.', 'logindesignerwp'),
     ...
     * This bypasses wp.media to avoid conflicts with other plugins.
     */
    public function ajax_upload_image()
    {
        // Verify nonce
        if (!wp_verify_nonce($_POST['nonce'] ?? '', 'logindesignerwp_save_nonce')) {
            wp_send_json_error(array('message' => __('Security check failed.', 'logindesignerwp')));
        }

        // Check permissions
        if (!current_user_can('upload_files')) {
            wp_send_json_error(array('message' => __('You do not have permission to upload files.', 'logindesignerwp')));
        }

        // Check if file was uploaded
        if (empty($_FILES['file'])) {
            wp_send_json_error(array('message' => __('No file was uploaded.', 'logindesignerwp')));
        }

        // Require the WordPress media handling functions
     ...
     * This now outputs a React mount point - all UI is rendered by React.
     */
    public function render_settings_page()
    {
        ?>
        <div class="wrap">
            <div id="ldwp-admin-root"></div>
        </div>
        <?php
    }

    // Legacy PHP render methods below are kept for reference but not used when React is active.
    // TODO: These can be removed once React migration is fully complete.

    /**
     * Legacy: Render background section (kept for reference).
     *
                        ...
                        <tr>
                            ...
                            <td>
                                <label class="ldwp-toggle">
                                    <input type="checkbox"
                                        name="<?php echo esc_attr($this->option_name); ?>[background_overlay_enable]" value="1"
                                        <?php checked(!empty($settings['background_overlay_enable'])); ?>>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="hidden" name="<?php echo esc_attr($this->option_name); ?>[button_border_radius]"
                                value="<?php echo esc_attr($settings['button_border_radius']); ?>" class="ldwp-corner-value">
